...
export type HarmonyState = {
  ...
  settings: {
    ...
    gainAdjustments: Record<string, number>;
    isMuted: boolean;
...
export type HarmonyAction =
  ...
  | {
      type: "joined";
      id: string;
      data: { users: User[]; server: Server; channels: Channel[] };
    }
  | { type: "user"; user: User }
  ...
  users: (state = { viewer: null, entities: {} }, action) => {
    ...
    if (action.type === "face") {
      return produce(state, (state) => {
        ...
        let dir = {
          x: action.x - user.position.x,
          y: action.y - user.position.y,
        };
        if (dir.x == 0 && dir.y == 0) {
          return state;
        }
  ...
  reducer: (state, action) => {
    ...
    if (action.type === "move") {
      return produce(state, (state) => {
        ...
        for (const channelID of servers.entities[servers.activeServer]
          .channels) {
          const channel = state.channels[channelID];
          const cnt = [0, 0];
          for (let i = 0; i < channel.border.length; i++) {
            const p1 = channel.border[i],
              p2 = channel.border[(i + 1) % channel.border.length];
...
export const useHarmonySelector = useSelector as <Selected = unknown>(
