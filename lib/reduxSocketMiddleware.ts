...
export type SocketMiddleware = Middleware<{}, HarmonyState, Dispatch<HarmonyAction>>;
const reduxSocketMiddleware: SocketMiddleware = store => {
    let socket: HarmonySocket | undefined;
    let rtcData: RTCPoolData | undefined;

    const debouncedFace = debounce(({ x, y }: { x: number, y: number; }) => socket?.emit('face', { x: x, y: y }));
