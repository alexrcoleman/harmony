...
export default abstract class SocketIOServer {
  public static setupServer(server) {
    const io = new Server<ClientToServerEvents, ServerToClientEvents>(server);
    io.on("connection", (socket) => {
      SocketIOServer.handleSocket(io, socket);
    });
    return io;
  }
  ...
  static handleSocket(
    ...
    socket.on("login", (id, cb) => {
      ...
      if (user != null) {
        ...
        console.log(
          socketUserID + " joined " + activeServer + " [" + socket.id + "]"
        );
        ...
        const users = server.users
          .map((id) => Entity.getUser(id))
          .filter(Boolean) as User[];
        const channels = server.channels
          .map((id) => Entity.getChannel(id))
        ...
        for (let userID of server.users) {
          ...
          io.to(socketId).emit("addPeer", {
            peer_id: socket.id,
            should_create_offer: false,
          });
          io.to(socket.id).emit("addPeer", {
            peer_id: socketId,
